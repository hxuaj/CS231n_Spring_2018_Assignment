...
def affine_backward(dout, cache):
    ...
    db = np.dot(np.ones(np.shape(x)[0]), dout)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dw, db
...
def batchnorm_backward(dout, cache):
    ...
    x_var = np.reshape(x_var, (1, -1))

    N = np.shape(x_hat)[0]
    # when I try to replace the operation of all-one vector with np.sun(_, axis=0), 
    # I found the speed is slower.
    one_row = np.ones((1, N))  # 1^T 
    one_vec = np.ones((N, 1))  # 1

    dbeta = np.dot(one_row, dout)
    dgamma = np.dot(one_row, np.multiply(dout, x_hat))

    dxnorm = np.multiply(dout, np.dot(one_vec, gamma))
    var_vec = np.dot(one_vec, 1/np.sqrt(x_var + eps))
    dx_part1 = np.multiply(dxnorm, var_vec)
    dx_part2 = - 1/N * np.dot(np.dot(one_vec, one_row), (np.multiply(dxnorm, var_vec)))

    a = np.dot(one_row, np.multiply(dxnorm, (x - x_mean)))
    b = -1/2 * np.power((x_var + eps), -3/2)
    dx_part3 = 2/N * np.multiply(np.dot(one_vec, np.multiply(a, b)), x - x_mean)
...
def batchnorm_backward_graph(dout, cache):
    ''' This is the BN backward using computation graph composed of simple 
        operations and backprop through all intermediate values.'''

    dx, dgamma, dbeta = None, None, None

    N, D = np.shape(dout)
    x, x_hat, x_mu, var, gamma, _, eps = cache
...
def batchnorm_forward_test(x, gamma, beta, eps):
    '''
    This is the additional bn implement for comparison
    '''
    # Step 1
    mu = np.mean(x, axis=0)
...
def batchnorm_backward_alt(dout, cache):
    ...
    N, D = np.shape(dout)
    x, x_hat, x_mu, x_var, gamma, _, eps = cache

    dxhat = dout * gamma
    dx = (1. / N) * (1.0 / np.sqrt(x_var + eps)) * (N*dxhat - np.sum(dxhat, axis=0) \
    ...
    # dbeta = np.sum(dout, axis=0)
    # dx_hat = dout * gamma
    # dx =  1.0 / np.sqrt(x_var + eps) * (dx_hat - 1.0/N * np.sum(dx_hat, axis=0) - (x - x_mean)/(N * (x_var + eps)) * np.sum(dx_hat * (x-x_mean), axis=0))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    # change slightly on the BN code
...
def layernorm_backward(dout, cache):
    ...
    # dxhat = dout * gamma
    # dx = (1. / D) * (1.0 / np.sqrt(x_var + eps)) * (D*dxhat - np.sum(dxhat, axis=1).reshape(-1, 1) 
    #                                                 - x_hat*(np.sum(dxhat*x_hat, axis=1).reshape(-1, 1)))
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(H_out):
        for j in range(W_out):
            x_slide = x_pad[:, :, (i*stride):(i*stride+HH), (j*stride):(j*stride+WW)]
            z[:, :, i, j] = np.tensordot(x_slide, w, axes=([1,2,3],[1,2,3])) + b
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(H_out):
        for j in range(W_out):
            dx_pad[:, :, (i*stride):(i*stride+HH), (j*stride):(j*stride+WW)] += np.tensordot(
                dout[:, :, i, j], w, axes=(1, 0)
            )#(4,2,1,1) dot (2,3,3,3) = (4,3,3,3)
            # dw += np.tensordot(
            #     x_pad[:, :, (i*stride):(i*stride+HH), (j*stride):(j*stride+WW)],
            #     dout[:, :, i, j], axes=(0, 0)
            # ).transpose(3, 0, 1, 2)#(4,3,3,3) dot (4,2,1,1) = (3,3,3,2) -> (2,3,3,3)
            dw += np.tensordot(
                dout[:, :, i, j],
                x_pad[:, :, (i*stride):(i*stride+HH), (j*stride):(j*stride+WW)],
                axes=(0, 0)
            )#(4,2,1,1) dot (4,3,3,3) = (2,3,3,3)
            # The shape of dout[:, :, i, j] is actually 2-D, so the tensordot result
            # is (1+3)D=4D -> (2,3,3,3)
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            # Find out the maximum value's index in the filter block.
            index = np.asarray(np.where(x_slice == out[:, :, i, j].reshape(N, C, 1, 1)))
            # Shift the index with filter. The index here is the coordinates of x.
            index += np.array([0,0,i*stride,j*stride]).reshape(-1,1)
            mask[tuple(index)] += 1
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(H_out):
        for j in range(W_out):
            mask[:, :, (i*stride):(i*stride+ph), (j*stride):(j*stride+pw)] *= dout[:, :, i, j].reshape(N, C, 1, 1)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    dx = (1. / N_r) * (1.0 / np.sqrt(x_var + eps)) * (N_r*dxhat - np.sum(dxhat, axis=0) 
         - x_hat*np.sum(dxhat*x_hat, axis=0))
    dx = dx.T
