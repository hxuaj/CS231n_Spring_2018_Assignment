...
def conv_forward_strides(x, w, b, conv_param):
    ...
    # how to compute strides: just consider the gap in each dimension of the new shape.
    # Strides describe, in any dimension, how many bytes we need to jump over 
    # in the data buffer to go from one item to the next.
    strides = (H * W, W, 1, C * H * W, stride * W, stride)
    strides = x.itemsize * np.array(strides)
    # shape: the shape of the new array
    # strides: the strides of the new array
    # 这里strides：用原数组维度表示的新的数组相邻“元素”之间在各个新dimension上的位置差异。
    # as_strided does not check that you stay inside the memory block bounds.
    x_stride = np.lib.stride_tricks.as_strided(x_padded,
                  shape=shape, strides=strides)
    # np.ascontiguousarray Return a contiguous array in memory in C order == Row-major order
    # relatively, F order == Column-major order
